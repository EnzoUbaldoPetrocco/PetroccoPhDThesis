...
The second part of this thesis proposes a novel framework for embedding cultural competence in SRs. It includes the experiments designed and conducted during my research stay at ETH Zurich, in which an implementation of the proposed framework was deployed on the Pepper SR.

This chapter describes the proposed framework and its constituent components. In addition, it presents the implementation evaluated on the Pepper SR.
...
\begin{itemize}
    ...
    \item Cultural Initial Preferences and User Preferences Components: These components act as memory repositories. The Cultural Initial Preferences component stores the user's national culture or initial preference profile, while the User Preferences component stores the preferences learned and updated by the Cultural Adapter component. These preferences are represented as cultural parameters that influence the entire processing pipeline.
...
To facilitate the understanding of certain design choices explained below, we anticipate here that two national cultures were selected for the study: Germans and Italians. The rationale for this choice is presented in the Participants section (Section~\ref{sec:participant}) in the next chapter.
